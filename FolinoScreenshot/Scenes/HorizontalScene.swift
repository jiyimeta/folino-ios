...
/// The Reader rendered in horizontal-scroll layout. Identical to `ReaderScene` except it forces
/// `ReaderLayoutMode.horizontal` (the Reader reads `ReaderGlobalSettingsKey.layoutMode` from `UserDefaults` to pick its
/// container) and uses its own copy keys.
...
#Preview(traits: .appStoreIPhone) {
    HorizontalScene().environment(\.screenshotIdiom, .iPhone)
}
