...
- **`App/`** — composition root, made up of `App/Shared` plus one platform shell per app target (`App/iOS` for `Folino`, `App/Mac` for `FolinoMac`). These app targets remain the only place Infrastructure adapters are wired into Feature view models; no Feature may compose Infrastructure itself. `FolinoMac` is a second application target; today it compiles only `App/Mac`, and gains `App/Shared` once the Feature packages `App/Shared` depends on (starting with `Reader`) build for macOS.
