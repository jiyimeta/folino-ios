...
| letter input on a chord's upper notehead | `Packages/Features/Editor/Sources/Editor/EditorViewModel+Input.swift:253` | Android's `.writeNote` path re-pitches notehead 0; Android still needs the caret-notehead `.setNotePitch` branch iOS keeps here. |
