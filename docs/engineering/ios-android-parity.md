...
| library → reader navigation seam | `App/Mac/MacShellView.swift:128` | LibraryRootScreen's readerDestination closures exist for the iOS NavigationStack push. On the Mac the detail column owns the reader, so these are never entered. If a Mac ever needs an in-sidebar push (a playlist drill-down that opens a score in place), this is where it hooks in. |
| playlist context in the reader | `App/Mac/MacShellView.swift:138` | the window carries a score id and nothing else, so `MacReaderRootScreen` opens the score standalone and the playlist's continuation control and end-of-score auto-advance are unreachable. Threading a `PlaylistID` through `MacWindowScore` is what closes it. |
| score playback from the Mac reader | `App/Mac/MacShellView.swift:174` | `AudioStack.playbackController` is nil until a later task builds the AVAudioSession-free controller, so the transport has nothing to drive. |
...
| page-mode reader extras | `Packages/Features/Reader/Sources/Reader/Screens/Mac/MacPagedScoreContainer.swift:1` | the Mac deck draws every page and lets the scroll view magnify them, and nothing else. The iOS `PagedScoreContainer`'s page-turn tap zones, swipe-to-turn, the PencilKit annotation overlay, and the note-editing seam have no Mac equivalent yet; the deck also has no counterpart to the reader's pinch-zoom commit, because zoom lives on `NSScrollView.magnification` here instead. |
| the Mac reading surface's chrome | `Packages/Features/Reader/Sources/Reader/Screens/Mac/MacReaderRootScreen.swift:1` | this screen renders the score in Page or Vertical mode and lets it scroll, and nothing else. The transport, the inspectors, the share / annotate / edit controls, Horizontal layout mode, and the original-PDF renditions are all still iOS-only; see `ReaderRootScreen` for the surface being caught up to. |
| PDF renditions in the reader | `Packages/Features/Reader/Sources/Reader/Screens/Mac/MacReaderRootScreen.swift:188` | `PagedPDFContainer` / `VerticalPDFContainer` are `PDFKit` + `UIScrollView` surfaces gated to iOS, so a PDF-backed item has nothing to draw on the Mac and says so instead. The same gap covers `displaySource == .originalPDF` for an item folino DID read into notation — the Mac has no way to switch to the original. |
| vertical reader extras | `Packages/Features/Reader/Sources/Reader/Screens/Mac/MacVerticalScoreContainer.swift:1` | the Mac container renders and scrolls the score, nothing else. Pinch zoom (`PinchState` / `ScoreScrollHost`), the PencilKit annotation canvas, and the note-editing overlay all live in the iOS `VerticalScoreContainer`'s UIKit-hosted subtree and have no Mac equivalent yet. |
...
| SettingsSheet dismiss chrome | `Packages/Utility/Sources/UtilityUI/PlatformToolbarCompat.swift:102` | iOS's Done button assumes the screen is always presented as a dismissible sheet; a macOS `Settings` scene hosts the same content directly, with no presentation to dismiss, so this drops the button rather than adapting it. |
