...
| library → reader navigation seam | `App/Mac/MacShellView.swift:92` | LibraryRootScreen's readerDestination closures exist for the iOS NavigationStack push. On the Mac the detail column owns the reader, so these are never entered. If a Mac ever needs an in-sidebar push (a playlist drill-down that opens a score in place), this is where it hooks in. |
