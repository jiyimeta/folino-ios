...
| the Mac reading surface's chrome | `Packages/Features/Reader/Sources/Reader/Screens/Mac/MacReaderRootScreen.swift:1` | this screen renders the score and lets it scroll, and nothing else. The transport, the inspectors, the share / annotate / edit controls, Page and Horizontal layout modes, and the original-PDF renditions are all still iOS-only; see `ReaderRootScreen` for the surface being caught up to. |
| PDF renditions in the reader | `Packages/Features/Reader/Sources/Reader/Screens/Mac/MacReaderRootScreen.swift:159` | `PagedPDFContainer` / `VerticalPDFContainer` are `PDFKit` + `UIScrollView` surfaces gated to iOS, so a PDF-backed item has nothing to draw on the Mac and says so instead. The same gap covers `displaySource == .originalPDF` for an item folino DID read into notation — the Mac has no way to switch to the original. |
