...
| erasable ink in exported PDFs | `Packages/Features/Reader/Sources/Reader/Annotation/Export/AnnotatedPDFComposer.swift:26` | the AKAnnotationV2 encoder is already shared in ReaderAnnotationCore, but Android has no PDF export to call it from, and would need a Deflating implementation over zlib. |
