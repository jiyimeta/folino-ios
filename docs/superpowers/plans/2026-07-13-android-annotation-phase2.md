...
**Status (2026-07-16): DONE + device-verified.** A1 (`941488d3`) added the shared `AnnotationAnchoringCore` + `AnchorResolving` + `StrokeTransform` + `LayoutDocumentAnchorResolver` + round-trip tests (pure addition). A2 (`5091855b`) routed iOS `AnnotationAnchoring.{capture,display,partitionByPage,anchorPoint}` through the core (behavior-preserving dual path: masked→legacy PKDrawing, unmasked→core byte-equivalent; read-both preserved). Verified: all 6 annotation suites (29 tests) green on the iPhone 17 Pro Max sim, **and on an iPad mini (6th gen) device** — draw / reflow-follow / persist / cross-mode / paged / zoom all OK, no regression. No stored-format change / no migration (existing DB blobs untouched). Sub-plan A is a self-contained device-verified refactor and can merge to main independently of C–F. The paged band-space variants (`capturePaged`/`displayPaged`) and `PDFAnnotationAnchoring` (iOS-only, no Android PDF) were intentionally left on their proven path — a later A3 increment can route paged through the core if desired.
