#!/usr/bin/env bash
# Build FolinoEditorJNI for each enabled Android ABI and stage .so files
# (plus Swift runtime stubs) into Android/FolinoEditorAndroid/src/main/jniLibs/,
# and the swift-java-generated Java bindings into src/main/java-generated/.
#
# The Swift JNI target lives in the Editor feature package: FolinoEditorJNI →
# EditorCore (the shared editing session) and swift-sheet-music's
# SheetMusicEditWire (the one intent schema). PKG_PATH points at
# Packages/Features/Editor while ROOT is the repo root (used only for the
...
# SWIFT_SHEET_MUSIC_ANDROID, not FOLINO_ANDROID). FolinoEditorJNI links SheetMusicEditWire, so without
# this the cross-compile drags in the Apple-only ssm targets and fails on `no such module CoreGraphics`.
export SWIFT_SHEET_MUSIC_ANDROID=1

ROOT="$(cd "$(dirname "$0")/.." && pwd -P)"
PKG_PATH="$ROOT/Packages/Features/Editor"
JNI_DIR="$ROOT/Android/FolinoEditorAndroid/src/main/jniLibs"
...
for entry in "${TARGETS[@]}"; do
    ...
    echo
    echo "==> Building libFolinoEditorJNI.so for $abi ($triple)"
    swift build --package-path "$PKG_PATH" \
                --product FolinoEditorJNI \
                --swift-sdk "$triple" \
                -c release

    src_so="$PKG_PATH/.build/$triple/release/libFolinoEditorJNI.so"
    dst_dir="$JNI_DIR/$abi"
    rm -rf "$dst_dir"
    mkdir -p "$dst_dir"
    cp "$src_so" "$dst_dir/"
...
# by the lowercased package identity ("editor"), not the on-disk dir name.
pkg_id="$(basename "$PKG_PATH" | tr '[:upper:]' '[:lower:]')"
GEN_JAVA_SRC="$PKG_PATH/.build/plugins/outputs/$pkg_id/FolinoEditorJNI/destination/JExtractSwiftPlugin/src/generated/java"
GEN_JAVA_DST="$ROOT/Android/FolinoEditorAndroid/src/main/java-generated"
...
echo
echo "Done. libFolinoEditorJNI.so + libSwiftJava.so + runtime staged under:"
