...
/// Snapshot of the CloudKit sync engine's state, exposed via an `AsyncStream` so UI can drive a status indicator.
...
/// Drives CloudKit Private Database sync of `ScoreItem`, `Tag`, `Playlist`, `AnnotationLayer`, and
/// `PlaybackPreferences`. Always-local invariant (`docs/product/feasibility.md` D4) lives in the Infrastructure
