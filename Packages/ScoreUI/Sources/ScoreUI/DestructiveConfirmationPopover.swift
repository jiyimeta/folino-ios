...
extension View {
    ...
    ///
    /// The cost, accepted knowingly: a popover is anchored to a view, so it goes down with that view. The editing
    /// strip's two confirmations used to live on its stable root precisely so a `ViewThatFits` refold or a
    /// cutout-tier toggle could not tear them down mid-question. Anchoring is worth more than surviving a rotation
    /// taken while the question is on screen — and the controls that raise these are the ones that never fold.
    ///
    ...
    public func destructiveConfirmationPopover(
...
#if DEBUG
// A short message and a long one, at the real width. The long case is what would be cut off if the sizing were
// wrong — it is the shape of the editing strip's revert confirmation with every caveat attached.
#Preview("Confirmation content") {
    VStack(spacing: 24) {
        DestructiveConfirmationContent(
            message: "Are you sure you want to\ndiscard your changes?",
            actionTitle: Text(verbatim: "Discard Changes"),
            action: {},
        )
        ...
        DestructiveConfirmationContent(
            message: [
                "Reverting to the original deletes every edit you have made to this score.",
                "Handwritten annotations anchored to the notation may shift.",
                "The original kept for this score may not be the file you imported.",
            ].joined(separator: "\n\n"),
            actionTitle: Text(verbatim: "Revert to Original"),
            action: {},
        )
        .background(.background, in: .rect(cornerRadius: 14))
    }
