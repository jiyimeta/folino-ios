...
struct TagRecord: FetchableRecord, PersistableRecord, Codable {
    ...
    ///
    /// `colorHex` is nullable in SQLite (color is optional from the user's
    /// perspective) but `Domain.Tag.colorHex` is non-optional `String`. The
    /// codebase convention is "empty string means no color"; null DB values
    /// map to "" here so consumers don't need to special-case missing colors.
