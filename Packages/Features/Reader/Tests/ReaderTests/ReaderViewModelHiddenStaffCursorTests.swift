...
struct ReaderViewModelHiddenStaffCursorTests {
    ...
    /// Two-staff piano-shaped score. Both staves have the same shape: one measure, one voice, two quarter chords (each
    /// a single note). Division 480 → quarter = 480 ticks. Element index 1 sits at tick 480.
    ...
    /// earlier staff in the same part is hidden). The `LayoutDocument` is built from the filtered score, so its
    /// `NoteID`s carry filtered staff addresses. If the cursor isn't re-stamped to the filtered address,
    /// `PlaybackCursorView.itemFrame` fails to match any layout entry and the cursor visually disappears during
    /// playback.
    ...
    /// was built from it). `StaffAddress` is purely positional, so when the FIRST staff is hidden the visible staff
    /// renders at filtered address `(0, 0)` but its full-score address is `(0, 1)` — the address the playback engine's
    /// timeline is keyed by. Without translation the engine can't resolve the cursor (most visibly when the visible
    /// staff holds a whole rest and the hidden staff holds notes: the `.rest` key bucket collides with the hidden
    /// staff's `.note` entries and the seek silently no-ops). Verify the controller receives the full-score address.
    @Test func `manual cursor translates filtered staff address to full score`() async {
        ...
        // Hide the FIRST staff so the visible staff's filtered address (0, 0) disagrees with its full-score address (0,
        // 1).
        let hiddenStaff = StaffAddress(partIndex: 0, staffIndexInPart: 0)
        await vm.layoutModel.toggleStaff(hiddenStaff)

        // What `nearestCursor` would produce after a tap on the visible staff: filtered address (0, 0).
    ...
    /// Whole-rest variant of the translation test — this is the case the user reported (visible staff = whole rest,
    /// hidden staff = notes).
    ...
    /// No translation when nothing is hidden — the cursor's filtered address already matches the full-score address.
