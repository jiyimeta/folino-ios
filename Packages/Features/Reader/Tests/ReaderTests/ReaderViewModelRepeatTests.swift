...
extension ReaderGlobalSettingsTests {
    ...
    struct ReaderViewModelRepeatTests {
        ...
        @Test func `set repeat B snaps to cursor measure end`() async {
            ...
            // FakeScoreFileGateway loads a 6-measure score; m1 has 2 chord positions (indices 0 and 1), so the
            // snapped end ChordPath has measureIndex == 1 and chordIndex == 1. B-only → loop is incomplete;
            // abRepeat is nil; pendingRepeatB reflects the snap.
        ...
        @Test func `clear repeat A removes start but keeps end`() async {
            ...
            // Once start is cleared and end remains, the persisted record drops the range entirely (loop is
            // incomplete) but keeps the B marker for re-display via a separate `pendingB` accessor.
            #expect(vm.repeatModel.abRange == nil)
            #expect(vm.repeatModel.pendingRepeatB?.measureIndex == 1)
        }
        ...
        @Test func `persisted ab loop forwards loop range to controller on prep`() async {
            ...
            // Repeat mode is now global; the per-score `repeatMode` above is ignored on load, so set the global
            // mode too.
