...
extension ScoreCursor {
    /// Measure index this cursor is parked on, regardless of `.item` vs `.beat` flavour. Used by `RepeatModel`'s A/B
    /// toggle logic to ask "is this loop boundary already on the same measure as the playback cursor?".
