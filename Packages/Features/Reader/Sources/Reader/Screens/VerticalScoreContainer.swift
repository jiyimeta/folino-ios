...
/// whenever the score, staff size, or container width changes. Holding the document on this view (instead of letting
/// `ScoreView`'s convenience init re-run layout each pass) keeps re-layout cost confined to real input changes — and
/// makes the document available to a future `ScoreHitTester` without rebuilding.
///
/// Drives playback auto-scroll: when `playbackCursor` moves outside the viewport in either axis, an animated scroll
/// command brings the cursor's frame back inside with a small padding inset. Horizontal follow only kicks in once
/// `viewportZoom` makes the content wider than the viewport — at zoom 1.0 the score wraps to fit and X never moves.
///
/// Owns the pinch / double-tap zoom gestures. The score content is `scaleEffect`-ed *inside* the scroll host (with an
/// explicit scaled `.frame`) so the underlying `UIScrollView` is never zoomed — its `maximumZoomScale` is pinned at 1
/// and it scrolls the zoomed extent natively. That keeps the SwiftUI `Canvas` re-rasterising under `scaleEffect` (sharp
/// throughout the pinch) instead of falling back to a `CALayer` bitmap upscale.
///
...
/// On gesture end the live factor is folded into `viewportZoom` and a scroll command is queued so the pinch's content
/// point stays under the same viewport coord — equivalent to UIScrollView's `viewForZooming` behaviour but driven from
/// SwiftUI state so resolution doesn't drop.
struct VerticalScoreContainer: View {
    ...
    /// Programmatic scroll command consumed by `ScoreScrollHost`. The host applies the offset synchronously inside
    /// `updateUIView` (after forcing `layoutIfNeeded()` so the new `contentSize` derived from `viewportZoom` is in
    /// place) and clears the binding on the next runloop tick.
    @State private var pendingScroll: ScoreScrollCommand?
    /// `UIScrollView.adjustedContentInset.top` — kept around for any cursor-frame math that needs to know how much of
    /// the viewport is hidden behind nav chrome. The pinch commit math itself is expressed in `contentOffset` units
    /// (which already account for inset), so we don't add this back any more.
    @State private var contentInsetTop: CGFloat = 0
    /// System top safe-area inset (status bar / notch). Parent `ReaderRootScreen`'s `.safeAreaPadding(.top,
    /// ReaderTopOverlay.height)` augmentation is subtracted back out below, so this carries the system value only.
    @State private var safeAreaTop: CGFloat = 0
    /// Live pinch state, held as an `@Observable` reference so mutations propagate into the hosted score subtree via
    /// SwiftUI observation — not through `ScoreScrollHost.updateUIView`'s `rootView` reassignment, which drops the
    /// animation transaction set by `withAnimation { … }`. See `PinchState`'s docblock. The container's body does not
    /// read `pinch.*` directly; only `VerticalZoomedSurface` does.
    ...
    /// value immediately. See `HorizontalScoreContainer` for the reasoning — `expectedContentSize`'s closure reads this
    /// rather than `viewModel.viewportZoom` to dodge SwiftUI's animated interpolation while the commit transition is in
    /// flight.
    @State private var committedZoom: CGFloat = 1.0

    /// Vertical padding that lives inside the scaled content so the first / last system don't butt up against the
    /// viewport edges. Scales with the score because it's applied inside the `scaleEffect`. No horizontal counterpart:
    /// at zoom 1.0 we want the score to span the full viewport width edge-to-edge.
    ///
    /// Top is larger than bottom so the first system clears the nav chrome / safe area when `ignoresSafeArea()` lets
    /// the score slide underneath.
    private let scoreTopPadding: CGFloat = 44
    private let scoreBottomPadding: CGFloat = 16

    /// Captured at `onPinchBegan` so the gesture-end commit knows the zoom that was in effect at the start of the pinch
    /// (independent of any concurrent state mutation).
    ...
    var body: some View {
        GeometryReader { proxy in
            // Hand the full viewport width to layout. Any horizontal overflow in the resulting `doc.size.width` (engine
            // right margin, spanners, ties) is absorbed by `effectiveZoom`'s fit-to-width factor below — so the user
            // always sees the entire system at user-zoom 1.0 with no side margin.
    ...
    private func scrollContent(viewport: CGSize) -> some View {
        ScoreScrollHost(
            ...
            expectedContentSize: {
                // Read `committedZoom` (the closure-safe mirror) so we don't pick up SwiftUI's animated interpolation
                // of `viewModel.viewportZoom` during a commit transition.
        ...
        // background full-bleed and represents safe area as `contentInsets` — we can't). The overlays sit in a ZStack
        // on top, so letting the score slide under them is the intended look.
        .ignoresSafeArea()
    ...
    /// User zoom scaled by a fit-to-width factor so the rendered content never exceeds the viewport horizontally at
    /// user-zoom 1.0. `LayoutEngine.layout` reports `doc.size.width = totalSystemExtent + 2*sp`; spanners / ties /
    /// playback chrome can also extend slightly past that. Rather than chase every contribution, we measure the actual
    /// `doc.size.width` and shrink to fit when it exceeds the viewport — the user-visible behaviour is "zoom 1.0 = fit
    /// width".
    ...
    private func commitPinch(
        ...
        if isBounceBack {
            // Rubber-band release: user pinched below 1.0 from a baseline of 1.0. No actual zoom or scroll commit —
            // just animate the inner `pinch.magnification` back to identity so the visual snap from compressed back to
            // layout is a smooth motion (matches `UIScrollView`'s bounce feel) instead of an abrupt jump that pulls
            // content away from the anchor.
            //
            // Important: keep `pinch.anchor` at the gesture's start anchor for the duration of the animation. Animating
            // it toward `.center` would interpolate the scale pivot mid-bounce, sliding the content visibly toward the
            // frame center — visible as "judder". At `magnification = 1.0` the anchor is irrelevant so we just leave
            // the stale value behind; the next pinch's `onPinchBegan` overwrites it.
            withAnimation(.smooth(duration: 0.18)) {
                pinch.magnification = 1.0
                pinch.offsetX = 0
            }
        } else {
            // See `HorizontalScoreContainer` — set the closure mirror outside `withAnimation` so the post-commit inset
            // and clamp ranges resolve correctly on the first render pass.
            committedZoom = targetZoom
            pendingScroll = .immediate(scrollToTarget)
            let snapToUnit = targetZoom <= 1.0
            if snapToUnit {
                // See `HorizontalScoreContainer` for the rationale. Synchronously snap `viewportZoom` to target and
                // compensate magnification (= `combined / target`) so visible scale is invariant across the commit;
                // then in the next runloop tick animate magnification → 1.0 so visible scale (`viewportZoom × mag`)
                // transitions monotonically from `combined` to 1.0 without the product-of-linear-curves bulge.
                ...
                // Real zoom-in / zoom-out. See HorizontalScoreContainer for the rationale: combined visible scale is
                // invariant across the commit (`baseZoom × gr.scale = targetZoom × 1.0`); interpolating each factor
                // separately would make their product bulge along the easing curve and read as an unwanted scale
                // animation. Snap the scale state, animate only the live offset reset, and only when the scroll view
                // can't absorb it.
                //
                // `postFramedWidth = min(doc.width, viewport.width) * targetZoom` mirrors `effectiveZoom`'s
    ...
    private func autoScroll(
        ...
        // Mirror `zoomedSurface`'s effective scale (user zoom × fit-to-width) so cursor-frame coordinates match the
        // rendered scroll-content size.
...
/// (the parent `VerticalScoreContainer` body never touches these properties, so it doesn't re-render and the
