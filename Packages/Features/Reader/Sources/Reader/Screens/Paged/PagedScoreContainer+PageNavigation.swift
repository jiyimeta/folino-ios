...
extension PagedScoreContainer {
    ...
    private func commitPageTurn(to target: Int) {
        ...
        // this projects the new page's band. The reactive `.onChange(of: pageState.pageIndex)` also fires, but not
        // reliably in step with the `withAnimation` turn, and its `applyDrawing` byte guard can't win the echo race —
        // this makes the handoff to the new page's ink immediate and race-proof.
