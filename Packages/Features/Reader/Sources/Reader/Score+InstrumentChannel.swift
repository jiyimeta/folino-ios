...
extension Score {
    /// GM bank declared for the part that owns `address`, or nil when the
    /// address points outside the score. All staves under a part share
    /// the part's instrument, so the staff index is ignored.
    ...
    /// GM program declared for the part that owns `address`, or nil when
    /// the address points outside the score.
    ...
    /// MIDI's 0…127 to the slider's 0…1. Returns nil when the score has
    /// no matching part — callers fall back to their own default. Mirrors
