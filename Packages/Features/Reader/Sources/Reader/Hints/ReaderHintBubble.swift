...
//
// What differs from synclick: anchors arrive as GLOBAL rects through `ReaderHintCoordinator` rather than as SwiftUI
// `Anchor<CGRect>` preferences, because half the controls hinted here are `ToolbarItem`s hosted by the navigation bar,
// and preferences do not cross that hosting boundary.

// MARK: - Anchor plumbing

extension View {
    /// Reports this view's frame as the anchor for `target`, so a hint pointing at it knows where to put its caret.
    /// Safe to attach unconditionally: nothing is drawn, and writes are change-guarded inside the coordinator.
    ///
    ...
    var body: some View {
        GeometryReader { proxy in
            if let hint = coordinator.presentedHint, let anchor = coordinator.anchor(for: hint.target) {
                // The anchor is global; the overlay's own global origin converts it into this view's space.
