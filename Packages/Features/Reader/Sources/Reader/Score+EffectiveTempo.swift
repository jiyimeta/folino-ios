...
extension Score {
    ...
    ///
    /// The Reader uses this for the tempo readout: `beatGlyph` and `beatsPerMinute` give "♩. = 80" as engraved, and the
    /// marking tracks the section the cursor sits in.
    func governingTempo(at cursor: ScoreCursor?) -> Tempo? {
    ...
    /// Quarter-note BPM in force at `cursor` — the stable axis the tempo slider operates in (the marking's beat unit
    /// doesn't change the slider, only the readout). Falls back to MuseScore's 120 BPM default when no marking governs
    /// the position. A `nil` cursor yields the opening tempo.
    func effectiveQuarterBpm(at cursor: ScoreCursor?) -> Double {
