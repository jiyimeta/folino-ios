...
/// Owns the Reader's visual / layout preferences slice — staff size,
/// break policy, hidden staves, per-staff clef overrides. Carved out of
/// `ReaderViewModel` so `VisualInspectorScreen` (and the score-rendering
/// pipeline at `ReaderRootScreen`) read the same source of truth without
/// touching the full view model.
///
/// Persistent fields mirror the matching `ReaderPreferences` fields; the
/// parent VM's `onChange` callback copies this slice back into the
/// preferences struct at save time.
@MainActor
@Observable
final class LayoutSettingsModel {
    ...
    /// Fires synchronously after `toggleStaff` mutates `hiddenStaves`, so
    /// the parent VM can re-translate the playback cursor against the
    /// new visibility before the saved-cursor read lands on a hidden
    /// staff. The async `onChange` still runs for persistence.
    ...
    /// Returns the rawType the renderer will use for the staff: the
    /// override if one is set, otherwise the score's authored opening
    /// clef, falling back to `"G"` if neither exists or the staff isn't
    /// in the score.
