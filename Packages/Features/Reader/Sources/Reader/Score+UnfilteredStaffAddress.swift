...
extension Score {
    ...
    /// Forward direction of `unfilterStaffAddress`: given a full-score
    /// `StaffAddress`, return the corresponding address in the filtered
    /// score, or `nil` when the staff is itself hidden (or its enclosing
    /// part is fully hidden).
    ///
    /// `PlaybackEngine`'s timeline is keyed by full-score addresses, but
    /// the Reader's `LayoutDocument` is built from the filtered score
    /// and stamps filtered addresses onto its `NoteID` / `RestID` keys.
    /// The cursor has to be re-stamped before the renderer can locate
    /// it — without this, a cursor emitted on a visible staff whose
    /// full address differs from its filtered address (any time an
    /// earlier staff in the same part is hidden, or an earlier part is
    /// fully hidden) silently fails the layout lookup and disappears.
