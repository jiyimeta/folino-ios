...
struct SelectionRederivationTests {
    @Test func `chord with a note re-derives to a note selection`() {
        ...
        #expect(item == .note(EditorFixtures.noteID(element: 1)))
    }

    @Test func `empty chord (a rest) re-derives to a rest selection`() {
        let score = EditorFixtures.chordAtIndex1()
        let location = VoiceElementID(staff: EditorFixtures.staff0, measureIndex: 0, voiceIndex: 0, elementIndex: 2)

        let item = SelectionRederivation.item(at: location, in: score, preferringNoteIndex: nil)

        #expect(item == .rest(EditorFixtures.restID(element: 2)))
    }

    @Test func `non-timed element re-derives to nil`() {
        let score = EditorFixtures.chordAtIndex1()
        let location = VoiceElementID(staff: EditorFixtures.staff0, measureIndex: 0, voiceIndex: 0, elementIndex: 0)
    ...
    @Test func `out of range element index re-derives to nil`() {
        let score = EditorFixtures.chordAtIndex1()
        let location = VoiceElementID(staff: EditorFixtures.staff0, measureIndex: 0, voiceIndex: 0, elementIndex: 99)
    ...
    @Test func `preferred note index clamps to the last note in a smaller chord`() {
        ...
        #expect(item == .note(EditorFixtures.noteID(element: 1, noteIndex: 0)))
    }
}
