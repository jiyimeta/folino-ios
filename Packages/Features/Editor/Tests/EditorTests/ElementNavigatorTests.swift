...
private typealias ElementNavigator = Editor.ElementNavigator
...
struct ElementNavigatorTests {
    ...
    @Test func `finds the previous rest in the same measure`() {
        let score = EditorFixtures.fourQuarterRests()
        let before = VoiceElementID(staff: EditorFixtures.staff0, measureIndex: 0, voiceIndex: 0, elementIndex: 2)

        let previous = ElementNavigator.previousTimedElement(before: before, in: score)

        #expect(
            previous == VoiceElementID(staff: EditorFixtures.staff0, measureIndex: 0, voiceIndex: 0, elementIndex: 1),
        )
    }

    @Test func `nil before the first timed element of the staff`() {
        let score = EditorFixtures.fourQuarterRests()
        let before = VoiceElementID(staff: EditorFixtures.staff0, measureIndex: 0, voiceIndex: 0, elementIndex: 1)
    ...
    @Test func `continues back into the previous measure's same voice`() {
        ...
        let before = VoiceElementID(staff: EditorFixtures.staff0, measureIndex: 1, voiceIndex: 0, elementIndex: 0)

        let previous = ElementNavigator.previousTimedElement(before: before, in: score)

        #expect(
            previous == VoiceElementID(staff: EditorFixtures.staff0, measureIndex: 0, voiceIndex: 0, elementIndex: 4),
        )
    }
    ...
    @Test func `skips non-timed elements when walking backwards`() {
        ...
        let before = VoiceElementID(staff: EditorFixtures.staff0, measureIndex: 1, voiceIndex: 0, elementIndex: 1)
    ...
    @Test func `next and previous are inverses`() {
        ...
        let start = VoiceElementID(staff: EditorFixtures.staff0, measureIndex: 0, voiceIndex: 0, elementIndex: 2)
