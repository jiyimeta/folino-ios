...
enum TiePlanner {
    ...
    /// can perform and no exporter can spell. `tieTarget` above answers a different question — "is there something
    /// here to tie TO" — and matches on pitch; this one follows the ties that already exist.
    ///
    /// Untied notes come back as a chain of one, so callers need no special case for the common shape.
    static func tieChain(containing noteID: NoteID, in score: Score) -> [NoteID] {
