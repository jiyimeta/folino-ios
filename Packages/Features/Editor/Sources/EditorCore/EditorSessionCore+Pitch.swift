...
extension EditorSessionCore {
    ...
    /// Writes a pitch onto `noteID` — and, because `.setNotePitch` addresses a tie chain as a unit, onto every note
    /// it is tied to, as one undo step.
    ///
    /// A tie chain is one sounding note written across several slots, so moving only the notehead under the finger
    /// produced something unplayable: two different pitches joined by a tie, still sounding at the original pitch
