...
struct LibraryStoreBulkTests {
    ...
    private static func makeStore(scoreItems: [ScoreItem] = []) -> Fixture {
        ...
        return Fixture(store: store, repo: repo)
    }

    // MARK: - bulkDelete

    @Test func `bulk delete removes all passed IDs`() async {
        let a = Self.makeItem(title: "A")
        let b = Self.makeItem(title: "B")
        let c = Self.makeItem(title: "C")
        let f = Self.makeStore(scoreItems: [a, b, c])

        await f.store.bulkDelete([a.id, c.id])
    ...
    @Test func `bulk delete empty is no op`() async {
        let f = Self.makeStore(scoreItems: [Self.makeItem(title: "A")])
        await f.store.bulkDelete([])
    ...
    @Test func `bulk delete stops at first error`() async {
        let a = Self.makeItem(title: "A")
        let b = Self.makeItem(title: "B")
        let f = Self.makeStore(scoreItems: [a, b])
        f.repo.deleteScoreItemError = .persistenceFailed(reason: "boom")

        await f.store.bulkDelete([a.id, b.id])

        #expect(f.repo.deletedScoreItemIDs.isEmpty) // FakeRepo throws before recording
        #expect(f.store.currentError == .domain(.persistenceFailed(reason: "boom")))
    }
    ...
    @Test func `bulk restore returns all to live`() async {
        ...
        await f.store.bulkRestore([a.id, b.id])
    ...
    @Test func `bulk permanently delete removes from both buckets`() async {
        ...
        await f.store.bulkPermanentlyDelete([a.id, b.id])
    ...
    @Test func `bulk remove from playlist filters and preserves order`() async {
        ...
        await f.store.bulkRemoveFromPlaylist([a.id, c.id], from: playlist)
    ...
    @Test func `bulk remove from playlist empty is no op`() async {
        let a = Self.makeItem(title: "A")
        let f = Self.makeStore(scoreItems: [a])
        let playlist = Playlist(
            name: "P", orderedScoreItemIDs: [a.id], createdAt: Self.base,
        )
        f.repo.playlists = [playlist]

        await f.store.bulkRemoveFromPlaylist([], from: playlist)
    ...
    @Test func `bulk add to playlist appends missing preserves order`() async {
        ...
        await f.store.bulkAddToPlaylist([b.id, a.id, c.id], to: playlist)
    ...
    @Test func `bulk add to playlist all present is no op`() async {
        ...
        await f.store.bulkAddToPlaylist([a.id], to: playlist)
    ...
    @Test func `bulk add to playlist empty is no op`() async {
        ...
        await f.store.bulkAddToPlaylist([], to: playlist)
    ...
    @Test func `bulk add tags unions tag sets`() async {
        let tagA = TagID()
        let tagB = TagID()
        var item1 = Self.makeItem(title: "1")
        item1.tagIDs = [tagA]
        var item2 = Self.makeItem(title: "2")
        item2.tagIDs = []
        let f = Self.makeStore(scoreItems: [item1, item2])

        await f.store.bulkAddTags([item1.id, item2.id], tagIDs: [tagB])
    ...
    @Test func `bulk add tags skips writes when already has all`() async {
        let tagA = TagID()
        var item = Self.makeItem(title: "1")
        item.tagIDs = [tagA]
        let f = Self.makeStore(scoreItems: [item])

        await f.store.bulkAddTags([item.id], tagIDs: [tagA])
    ...
    @Test func `bulk add tags empty tags is no op`() async {
        ...
        await f.store.bulkAddTags([item.id], tagIDs: [])
