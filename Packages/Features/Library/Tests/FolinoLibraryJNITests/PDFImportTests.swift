...
struct PDFImportTests {
    ...
    /// Same rule as iOS `LiveScoreFileImporter`: the title is the SOURCE FILE NAME, never the PDF's own `/Title`
    /// (this fixture carries `/Title` = "Sample Title", which must NOT win). Exporters bake their own project
    /// name into `/Title`, so the file name is the only signal the user actually chose. Both platforms go through
    /// the shared `ScorePresentation.displayFields(sourceFilename:)`.
    @Test func `title comes from the filename, not the document title`() throws {
        let backend = FakePDFImportStore()
        let store = makeStore(backend)
        _ = try store.importScore(fixtureURL().path)

        #expect(store.scores.first?.title == "sample")
    }
    ...
    @Test func `sharing a PDF lands a live pdf record`() throws {
        ...
        let record = try #require(backend.records.first)
        #expect(record.localFileName.hasSuffix(".pdf"))
        // The shared file name, not the document's own `/Title` — the same rule the picker applies.
